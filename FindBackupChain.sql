...
Select @DBBackupLSN = checkpoint_LSN -- Anders
From @Baks;
...
Select RestoreOrder = Row_Number() Over(Partition By family_sequence_number Order By BakID),
	...
	DeviceType = Case When device_type in (2, 102) Then 'Disk'
			When device_type in (5, 105) Then 'Tape'
		End,
	PhysicalFileName = replace(replace(physical_device_name, '\\agdata.int\backups\Bayer\BCS_Prod\', '\\BU\'), 'Audit', 'AndersTest')-- Audit\Audit_20150923.BAK
From @Baks
Order By BakID;
*/
